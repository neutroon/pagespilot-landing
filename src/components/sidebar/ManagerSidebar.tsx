...
export default function ManagerSidebar() {
  ...
  // Detect screen size and enforce non-collapsed on mobile
  useEffect(() => {
    const handleResize = () => {
      const mobile = window.matchMedia("(max-width: 1023px)").matches; // Tailwind lg breakpoint = 1024px
      setIsMobile(mobile);
  ...
  // Reset collapse when mobile sidebar is open
  useEffect(() => {
    if (isMobileOpen && isMobile) setIsCollapsed(false);
  ...
  const navigation: ManagerSidebarItem[] = [
    {
    ...
    },
    {
      ...
      href: "/manager/my-users",
      icon: <Users className="w-5 h-5" />,
    },
    {
      name: t("navigation.managers"),
      href: "/manager/my-managers",
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full sidebar-content">
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-200">
        ...
        <button
          onClick={() => !isMobile && setIsCollapsed(!isCollapsed)} // Only toggle on desktop
      ...
      <div className="flex-1">
        <div className="p-4 space-y-2">
          <div className="mb-4">
            ...
            >
              {isCollapsed ? "•" : "Manager Navigation"}
            </h3>
